...
export default function PrintBillPage() {
    ...
    return (
        <div className="mx-auto p-5 text-foreground print:text-gray-900 not-print:max-w-3xl" dir="rtl">
            {/* Header */}
            <div className="mb-6 border-b-[3px] border-orange-500 pb-5 text-center">
                ...
                <h1 className="mb-1 text-[22px] font-extrabold print:text-gray-900">ابو رقية للتجارة والتوزيع</h1>
            ...
            <div className="mb-5 flex justify-between rounded-lg border border-border/60 bg-muted/50 px-4 py-3 print:border-gray-200 print:bg-gray-50">
                <div className="text-[13px]">
                    <strong className="mb-0.5 block text-[11px] font-semibold text-muted-foreground print:text-gray-500">العميل</strong>
                    {bill.customerName}
                </div>
                <div className="text-[13px]">
                    <strong className="mb-0.5 block text-[11px] font-semibold text-muted-foreground print:text-gray-500">التاريخ</strong>
                    {date}
                </div>
            </div>
            ...
            <table className="mb-5 w-full border-collapse">
                <thead>
                    <tr>
                        <th className="rounded-r-lg bg-orange-500 px-3 py-2.5 text-right text-xs font-bold text-white">#</th>
                ...
                <tbody>
                    {bill.items.map((item, i) => (
                        <tr key={i} className="even:bg-muted/30 print:even:bg-gray-50">
                            <td className="border-b border-border/40 px-3 py-2.5 text-center font-mono text-[13px] print:border-gray-100">{i + 1}</td>
                            <td className="border-b border-border/40 px-3 py-2.5 text-[13px] print:border-gray-100">{item.name}</td>
                            <td className="border-b border-border/40 px-3 py-2.5 text-center font-mono text-[13px] print:border-gray-100">
                                {item.count}
                            </td>
                            <td className="border-b border-border/40 px-3 py-2.5 text-center font-mono text-[13px] print:border-gray-100">
                                {formatNumber(item.price)}
                            </td>
                            <td className="border-b border-border/40 px-3 py-2.5 text-center font-mono text-[13px] font-bold print:border-gray-100">
